...
#### generate build --prod
RUN ng build --configuration=production --output-hashing=none && ng run travels-ui:server:production --output-hashing=none
...
RUN pwd
RUN ls -a
CMD node ./dist/travels-ui/server/main.js